...
The Expo (React Native + web) successor to the Streamlit app in the sibling repo `coptic-transliterator-llm`: a **public, offline-first PWA** that transliterates Coptic text to Latin script (Greco-Bohairic conventions) so English speakers can follow along in Coptic Orthodox services. Phase 1 is fully client-side — **no backend, no auth, no database**. Phase 2 will add optional Gemini AI enhancement via Firebase AI Logic + App Check (free tier); Phase 3 may add Firestore for community features. Same Lumen design system as marketplace-selling and llc-tax-tracker (net-worth-tracker left it for its own flat "Ledger" look in Sep 2026).
...
- Lumen: `GlassCard/AppBackground/PressableScale/AnimatedNumber` are byte-identical with the sibling apps — run `~/dev_personal/bin/sync-lumen.sh --check` before editing, edit in one app, `--from <that-app>`, commit everywhere that carries the file. The behaviour components (`PressableScale`, `AnimatedNumber`) are shared by all four apps; the glass surfaces (`GlassCard`, `AppBackground`) only with marketplace-selling and llc-tax-tracker, since net-worth-tracker replaced them with its own `Panel`/`Backdrop`. `theme.ts` shares the token object by value with the other glass apps — which is why the behaviour components must never read `theme`; `Icon.tsx` and `Screen.tsx` are app-specific.
